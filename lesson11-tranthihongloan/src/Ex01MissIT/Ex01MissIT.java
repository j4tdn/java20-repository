...
public class Ex01MissIT {
	public static void main(String[] args) {
		...
		for (int i = 0; i < N; i++) {
			System.out.print("Nhap so luong thi sinh " + "giam khao thu " + (i + 1) + " chon: ");
		...
		// Tinh diem va so luot binh chon cua moi thi sinh
		...
		// Sap xep diem va so luot binh chon theo thu tu giam dan
		Collection<ScoreContestant> values = map.values();
		ScoreContestant[] array = values.toArray(new ScoreContestant[values.size()]);
		Arrays.sort(array, comparing(ScoreContestant::getScore, reverseOrder()).thenComparing(ScoreContestant::getnVote,
				reverseOrder()));

		// In thi sinh chien thang
