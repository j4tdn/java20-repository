...
public class ItemGroupServiceImpl implements ItemGroupService {
	...
	public List<ItemGroup> getItemGroupWithItems() {
		List<Item> items = itemDao.getAll(); // List<Item-ItemGroup>
		Map<ItemGroup, List<Item>> itemGroupItemsMap = items.stream()
	...
	public void save(Collection<ItemGroup> groups) {
		...
		if (!groups.isEmpty())
			for (ItemGroup itemGroup: groups)
				saveOrUpdate(itemGroup);
			
			// Cứ 1 item group -> gọi 1 hàm saveOrUpdate(service) -> save(dao)/update(dao)
			// -> tạo ra 1 [prepared]statement, executeUpdate (open/close connection liên tục)
			// JAVA: host, MySQL: host : bị ảnh hưởng bởi yếu tố network
			// --> nếu mạng có vấn đề --> hàm sẽ bị chậm
