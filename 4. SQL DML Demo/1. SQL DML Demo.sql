...
WITH CTE_MAX_AMOUNT_EACH_ITEM_GROUP AS (
	SELECT itg.ID ITEM_GROUP_ID, itg.NAME, MAX(itd.AMOUNT) MAX_AMOUNT
      FROM item_group itg
      JOIN item it
	  ...
	  JOIN item_detail itd
        ON itd.ITEM_ID = it.ID
	 GROUP BY itg.ID, itg.`NAME`
), CTE_MAX_AMOUNT_EACH_ITEM AS (
	SELECT it.ID, it.`NAME`, MAX(itd.AMOUNT) MAX_AMOUNT_ITEM, itg.ID ITG_ID
      FROM item_group itg
      JOIN item it
        ON it.ITEM_GROUP_ID = itg.ID
	  JOIN item_detail itd
        ON itd.ITEM_ID = it.ID
	  JOIN CTE_MAX_AMOUNT_EACH_ITEM_GROUP
        ON CTE_MAX_AMOUNT_EACH_ITEM_GROUP.MAX_AMOUNT = itd.AMOUNT
	   AND itg.ID = CTE_MAX_AMOUNT_EACH_ITEM_GROUP.ITEM_GROUP_ID
	 GROUP BY it.ID, it.`NAME`, itg.ID
) SELECT CTE_MAX_AMOUNT_EACH_ITEM.* 
    FROM CTE_MAX_AMOUNT_EACH_ITEM
    JOIN CTE_MAX_AMOUNT_EACH_ITEM_GROUP
      ON CTE_MAX_AMOUNT_EACH_ITEM.MAX_AMOUNT_ITEM = CTE_MAX_AMOUNT_EACH_ITEM_GROUP.MAX_AMOUNT
	 AND CTE_MAX_AMOUNT_EACH_ITEM.ITG_ID = CTE_MAX_AMOUNT_EACH_ITEM_GROUP.ITEM_GROUP_ID;
