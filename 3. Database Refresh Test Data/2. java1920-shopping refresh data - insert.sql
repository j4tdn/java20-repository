...
VALUES
	(1,1,4,2),	(2,1,31,5),	(3,1,49,1), (4,1,68,2),	
    (5,2,39,2), (6,2,48,2), (7,2,50,12), (8,2,53,3), 
    (9,3,21,2), (10,3,22,2), (11,3,39,2), (12,3,49,11),
	(13,4,11,2), (14,4,41,3), (15,4,46,3), (16,4,69,3),
	(17,5,5,3), (18,5,24,2), (19,5,39,8), (20,5,56,8),
	(21,6,10,2), (22,6,23,9), (23,6,29,3), (24,6,30,11), 
    (25,7,17,2), (26,7,43,15), (27,7,54,10), (28,7,57,5),
	(29,8,8,20), (30,8,18,7), (31,8,59,2), (32,8,65,9),
	(33,9,29,10), (34,9,34,1), (35,9,38,7), (36,9,53,2),
	(37,10,22,10), (38,10,24,10), (39,10,40,2), (40,10,50,2),
	(41,11,37,2), (42,11,38,2),(43,11,48,11), (44,11,51,2),
	(45,12,1,12), (46,12,29,2), (47,12,35,1), (48,12,42,3);

SELECT *
  FROM `ORDER` od, 
	    ORDER_STATUS ods
  WHERE od.ID IN (1,2,3,4,5);
...
SELECT od.ID ORDER_ID, 
  ...
  FROM `ORDER` od, 
	    ORDER_STATUS ods
  WHERE od.ID BETWEEN 6 AND 8 -- [1,5] inclusive
...
SELECT od.ID ORDER_ID, 
  ...
  FROM `ORDER` od, 
	    ORDER_STATUS ods
  WHERE od.ID BETWEEN 9 AND 10 -- [1,5] inclusive
...
SELECT od.ID ORDER_ID, 
	   ods.ID ORDER_STATUS_ID,
       FLOOR(1 + (rand() * (SELECT MAX(ID) FROM EMPLOYEE))) EMPLOYEE_ID,
       DATE_SUB(current_timestamp(), interval(7 - od.ID) DAY) UPDATED_AT
  FROM `ORDER` od, 
	    ORDER_STATUS ods
  WHERE od.ID BETWEEN 11 AND 12 -- [1,5] inclusive
    AND ods.ID IN (1,7)
)
SELECT @running := @running + 1 AS ID,
	   cte_data.*
