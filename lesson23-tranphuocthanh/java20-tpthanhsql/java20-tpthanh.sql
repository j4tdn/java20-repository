...
(
    ...
    -- Thiếu UNIQUE cho (CUSTOMER_ID, ROOM_ID, HOUR_RATE_ID)
);
...
(
     ...
     CONSTRAINT UNQ_ORDER_SERVICE UNIQUE (ORDER_ID, SERVICE_ID), -- bị thừa em nhé, PK thì cặp này đã UNIQUE rôồi
...
('HD006', 'DV01', 5);
 ...
 -- em mới xử lý là có sử dụng thôi, chứ chưa tìm ra được nhiều nhất theo thời gian sử dụng
 SELECT *
   FROM room r
   JOIN `order` od
     ON r.ID = od.ROOM_ID
